...
public class ExampleGame extends Game {
	...
	public void init(Window window) {
		// Create a matrix holding information about the projection of the camera, parameters are: FOV, aspect ratio, Z-Near and Z-Far
		...
		// Add the camera-component to the camera game-object, passing in the camera projection to the component
		camera.addComponent(new CameraComponent(cameraProjection));
		
		// Create a set of materials, there's no need to assign them to a variable, they're only loaded once
		new Material(
		...
		new Material(
				"bull",
				new Texture("example/bull_texture.jpg"),
				4,
				8,
				new Texture("example/bull_normal.jpg")
			);
		
		// Create a mesh renderer for the bull game-object, note that we create a material with the name "bull", this will cause it to use the previously created material
		MeshRenderer bullMesh = new MeshRenderer(new Mesh("example/bull.obj"), new Material("bull"));
		
		// When creating game-objects you can pass in the position, rotation and scale directly in the game object
		...
		// You can also create game-objects on one line, and adding them immediately
		addChild(
				new GameObject(new Vector3f(0, -1, 0))
	...
	private void addLights() {
		// Create a new directional light, with a light-gray color and an intensity of 0.1
		DirectionalLight directionalLight = new DirectionalLight(new Color(0.8f, 0.8f, 0.8f), 0.1f);
		
		// Create a new point light, with a cyan color, an intensity of 0.9 and an attenuation-exponent value of 1
		PointLight pointLight = new PointLight(new Color(0, 1, 0.5f), 0.9f, new Attenuation(0, 0, 1));
		
		// Create a new spot light, with a yellow color, a full intensity of 1, an attenuation-exponent value of 0.05 and a cutoff of 0.7
		...
		directionalLightObject.getTransform().setRotation(new Quaternion(new Vector3f(1, 0, 0), (float)Math.toRadians(-90)));
